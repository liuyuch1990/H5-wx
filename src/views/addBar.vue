    ...
    <div class="activityList">
        <div style="background: rgb(248, 248, 248);">
            <div class="active-wrap">
                <div v-for="item in bar" :key="item.id" class="item" :class="{'active': item.active || false}"
        ...
        <van-list
                ...
                @load="onLoad"
        >
            <div class="item" v-for="item in list" :key="item.id" :class="{'active': item.active}"
                 @click="choose(item)">
                <div class="img-wrap">
    ...
    export default {
        ...
        methods: {
            ...
            async choose(item) {
                ...
                if(item.active===true){
                    lst = !activeBar.value ? lst:activeBar.value.split(',')
                    lst.push(item.id)
                }else{
                    lst =  activeBar.value.split(',').length==1 ? activeBar.value.replace(item.id,'').split(','):activeBar.value.replace(','+ item.id,'').split(',')
                }
                let {data: res} = await this.$api.common.tempaleSave({
                    "code": activeBar.code,
                    "id": activeBar.id,
